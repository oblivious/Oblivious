	...
	sealed public class NameIdPermission : CodeAccessPermission, IUnrestrictedPermission
	{
		...
		public override bool IsSubsetOf(IPermission target)
		{
			...
			try
			{
				...
				if (operand.m_Unrestricted)
				{
					return true;
				}
				else if (this.m_Unrestricted)
				{
					return false;
				}
		...
		{
			NameIdPermission nameIdPermission = new NameIdPermission("Donal");
			IPermission otherPermission = nameIdPermission.Copy();

			nameIdPermission.IsSubsetOf(otherPermission);
			nameIdPermission.Intersect(otherPermission);
			nameIdPermission.Union(otherPermission);

			Console.WriteLine(nameIdPermission.ToXml());
