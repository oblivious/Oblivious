...
namespace BinderTest
{
    public class MyBinder : Binder
    {
            ...
            ParameterModifier[] modifiers, CultureInfo culture, string[] names, out object state)
        {
            ...
            for (int i = 0; i < match.Length; i++)
            {
                ...
                if (args.Length != parameters.Length)
                    continue;
                // Match each of the parameters that the user expects the methdo to have.
                for (int j = 0; j < args.Length; j++)
                {
                    // If the names parameter is not null, then reorder args.
                    // "names" is an array of strings representing the names of the parameters?
                    // The ordering may not match the actual ordering as passed in by the calling method.
                    if (names != null)
                    {
        ...
        {
            // Determine whether the value parameter can be converted to value of type myType.
            ...
            Type[] indexes, ParameterModifier[] modifiers)
        {
            ...
            for (int i = 0; i < match.Length; i++)
            {
                ...
                // Gon on to the next property if the number of indexes do not match.
        ...
        // Determines whether type1 can be converted to type 2. Check only for primitive types.
